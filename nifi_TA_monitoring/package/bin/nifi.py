...
class NiFiScript(Script):
        ...
        if auth_type == 'basic':
            try:
                if password != self.mask:
                    self._log(ew, EventWriter.INFO, '{} Encrypting/Mask password'.format(self.pid))
                    self.__encrypt_password(ew, username, password, session_key)
                    self.__mask_password(ew, session_key, input_name, input_item)
                else:
                    self._log(ew, EventWriter.INFO, '{} No Encrypting/Mask password'.format(self.pid))
            except Exception as e:
                self._log(ew, EventWriter.ERROR,'{} There was an error when encrypting/masking the password: {}'.format(self.pid, e))
    ...
    def __encrypt_password(self, ew, username, password, session_key):
        self._log(ew, EventWriter.INFO, '{} Init Encrypt Password'.format(self.pid))
        ...
        try:
            ...
            self._log(ew, EventWriter.INFO, '{} An error occurred updating credentials. Please ensure your user account has admin_all_objects and/or list_storage_passwords capabilities. Details: {}'.format(self.pid, e))
    ...
    def __mask_password(self, ew, session_key, input_name, input_item):
        self._log(ew, EventWriter.INFO, '{} Init Mask Password'.format(self.pid))
    ...
    def __get_password(self, ew, session_key, username):
        self._log(ew, EventWriter.INFO, '{} Retrieving stored credential'.format(self.pid))
        args = {'token': session_key}
        service = client.connect(**args)
        ...
        self._log(ew, EventWriter.ERROR, '{} No stored credential found for user {} - the input cannot authenticate. Re-save the input to store the password.'.format(self.pid, username))
        return None
